...
def test_get_goal_with_invalid_goal_id(valid_token):

    invalid_goal_id = faker.uuid4()
    response = send_request("GET", f"/goal/{invalid_goal_id}", valid_token)
...
def test_get_goal_with_invalid_auth_api_key(team_id, valid_token, invalid_token):
    ...
    #GET
    response = send_request("GET", f"/goal/{goal_id}", invalid_token)
