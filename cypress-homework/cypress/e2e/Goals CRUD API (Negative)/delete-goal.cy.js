...
describe('Delete Goal ClickUP endpoint', () => {
    ...
    it('DELETE deleted Goal, returns 404', () => {
    ...
    it('DELETE Goal with invalid goal_id, returns 404', () => {

        const invalidGoalId = faker.string.uuid();

        cy.env(['validToken']).then(({ validToken }) => {

            cy.sendRequest('DELETE', `/goal/${invalidGoalId}`, null, validToken)
    ...
    it('DELETE Goal with invalid auth_api_key, returns 401', () => {
        ...
        cy.env(['teamId', 'validToken', 'invalidToken']).then(({ teamId, validToken, invalidToken }) => {
            ...
            cy.sendRequest('POST', `/team/${teamId}/goal`, payload, validToken)
                ...
                .then((response) => {
                    ...
                    //DELETE
                    return cy.sendRequest('DELETE', `/goal/${goalId}`, null, invalidToken);
